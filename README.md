...
| 功能 | 说明 |
|---|---|
| **多仓库 Tab** | 主区域顶部 Tab Bar 同时打开多个仓库，每个 Tab 独立维护改动/diff/分支/历史/提交草稿/AI 对话 |
...
| **会话恢复** | 记住上次打开的 Tab 列表和激活项，下次启动自动恢复（路径失效时静默跳过该 Tab） |
...
| **Tab Bar** | 主区域顶部水平 Tab 条，每 Tab 显示仓库名+分支+关闭按钮，末尾"+"打开新仓库；空状态时显示"打开仓库"按钮 |
...
│ 改动│ │  M main.rs     │ +++ b/src/main.rs   │ │ -  let x = 0;  │
│ 分支│ │  A utils.rs     │ @@ -10,3 +10,5 @@   │ │                │
│ 审查│ │                 │ +fn new() -> Self { │ │                │
│ 对话│ │ 改动 (1)        │ +    Self { x: 1 }  │ │                │
│ 设置│ │  M README.md    │ +}                  │ │                │
│     │ ├─────────────────┴─────────────────────┤ │                │
│     │ │ [AI 生成]  提交信息...(Ctrl+Enter)    │ │                │
│     │ │  ↑1  2 已暂存  [提交并推送] [提交]    │ │                │
└─────┴────────────────────────────────────────────────────────────┘
```
...
[[recent_repos]]
open_repos = []                   # 启动时自动恢复的 Tab 路径列表
active_repo = ""                  # 上次激活的 Tab 路径，空表示无激活项
```
...
应用每次打开仓库会自动追加到列表首位，最多保留 10 条。设置页"最近仓库"区可直接点击条目在新 Tab 中打开。

#### `open_repos` / `active_repo` — 多仓库 Tab 会话

| 字段 | 说明 |
|---|---|
| `open_repos` | 当前打开的 Tab 路径列表（数组），按 Tab 顺序排列。每次 Tab 增减或切换都会持久化 |
| `active_repo` | 当前激活的 Tab 路径。关闭所有 Tab 后为空字符串 |

应用启动时会按 `open_repos` 顺序恢复所有 Tab，再激活 `active_repo` 指定的那一个；路径失效的 Tab 会被静默跳过。该字段独立于 `recent_repos`，不会因为关闭所有 Tab 而清空最近仓库列表。

## 功能演示

### 多仓库 Tab 管理

1. 点击主区域顶部 Tab Bar 末尾的 `+` 按钮，选择一个 Git 仓库目录
2. 新仓库会在新 Tab 中打开，自动激活并加载改动/分支/历史
3. 点击不同 Tab 切换激活的仓库，每个 Tab 独立保留：
   - 提交信息草稿（切 Tab 不会丢失输入中的内容）
   - 选中的文件和 diff 状态
   - 分支和历史浏览位置
   - AI 对话历史和代码审查结果
4. 鼠标悬停 Tab 显示关闭按钮，点击关闭；关闭激活 Tab 时自动切到相邻 Tab
5. 关闭应用后再次启动，所有 Tab 列表和上次激活的 Tab 会自动恢复
...
│   ├── stores/
│   │   ├── aiStore.ts                # AI 状态（生成/审查/对话，按仓库隔离）
│   │   └── repoStore.ts              # 多仓库 Tab 状态（含 push/pull/discard，每 Tab 独立）
│   ├── i18n/
│   │   ├── index.ts                  # i18next 配置
│   │   └── locales/
│   │       ├── zh.json               # 中文翻译
...
| MSI 安装包 | `src-tauri/target/release/bundle/msi/aigit_1.0.2_x64_en-US.msi` | ~6 MB | Windows Installer，支持组策略部署 |
| NSIS 安装包 | `src-tauri/target/release/bundle/nsis/aigit_1.0.2_x64-setup.exe` | ~4 MB | 轻量安装包 |
...
### Q: 启动时为什么自动打开了上次的多个仓库？

A: 这是 [会话恢复](#核心特性) 功能。配置文件 `open_repos` 字段记录了上次打开的 Tab 列表，`active_repo` 记录激活的那一个。应用启动时会按顺序恢复这些 Tab，路径失效的会被静默跳过；旧的 `recent_repos[0]` 字段仅在 `open_repos` 为空时作为回退使用。

### Q: 关闭 Tab 后再打开，提交信息草稿还在吗？

A: 不在。关闭 Tab 会清除该 Tab 的全部状态（改动、diff、分支、历史、提交草稿、AI 对话）。仅 `open_repos` / `active_repo` 会即时持久化到配置文件，方便下次启动恢复 Tab 列表本身。

### Q: 切换 Tab 时 AI 对话历史会丢失吗？

A: 不会。`aiStore` 的 `chatByRepo` 和 `lastResultByRepo` 按 repo path 隔离存储，切换 Tab 只会改变 `currentPath` 的指向，对应仓库的对话和审查结果会立即显示。
