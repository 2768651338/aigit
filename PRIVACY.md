...
生效日期：2026-08-01。本文描述 aigit 当前版本的数据行为，版本历史见 [CHANGELOG.md](CHANGELOG.md)。aigit 本身不运营遥测或账户服务，但用户选择的 Git、GitHub 和 AI 服务会按其各自政策处理数据。
...
- 本地应用数据目录：代码索引 JSON、聊天历史和审查结果等功能数据。设置页可以关闭/清除聊天历史、删除当前仓库索引。Windows 上聊天历史与代码索引使用 DPAPI（当前用户范围）加密存储；其他平台为明文文件。
- Git 仓库及 Git 配置：由本机 libgit2/系统 Git 读取和修改。

路径、提交作者和对话可能属于个人信息。Windows 上的 DPAPI 只保护当前用户账户，其他平台或被提权的程序仍可能访问本地文件；请使用操作系统磁盘/账户保护。
